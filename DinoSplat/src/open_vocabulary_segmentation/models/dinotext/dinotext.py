...
logger = logging.getLogger("dinotext")
...
class DINOText(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "dinov2" in model_name:
            ...
            logger.info(f"model_family: {self.model_family}, model_name: {model_name}")
        ...
        logger.info(f"resize_dim: {resize_dim}")
    ...
    def process_self_attention(
        ...
        logger.info("process_self_attention called")
        ...
        logger.info(f"self_attn_maps shape: {self_attn_maps.shape}")
        self_attn = self_attn_maps.mean(dim=1)
        self_attn = self_attn.softmax(dim=-1)
        logger.info(f"mean-reduced & softmaxed self_attn shape: {self_attn.shape}")
    ...
    def build_dataset_class_tokens(self, template_set, classnames):
        logger.info("build_dataset_class_tokens called")
        logger.info(f"classnames: {classnames}")
        logger.info(f"templates: {template_set}")
        tokens = []
    ...
    def build_text_embedding(self, text):
        ...
        Returns:
            text_embs
        """
        logger.info("build_text_embedding called")
        text = text.to(device)
        num_classes, num_templates = text.shape[:2]
        text_argmax = text.argmax(dim=-1)
        logger.info(f"text_argmax shape: {text_argmax.shape}")
        logger.info(f"text_argmax: {text_argmax}")
        text_argmax = rearrange(
            text_argmax, "n t -> (n t)", n=num_classes, t=num_templates
        )
        logger.info(f"rearranged text_argmax: {text_argmax}")
        logger.info(f"text shape: {text.shape}")
        text = rearrange(text, "n t l -> (n t) l", n=num_classes, t=num_templates)
        logger.info(f"rearranged text shape: {text.shape}")

        # chunked inference for memory limitation
        chunk_size = 32
        N = text.size(0)
        logger.info(
            f"type(self.proj) == CLIPLastLayer: {type(self.proj) == CLIPLastLayer}"
        )
        if type(self.proj) == CLIPLastLayer:
            text_embs = torch.cat(
                [
            ...
            if not self.use_avg_text_token:
                ...
                logger.info(f"text_embs shape: {text_embs.shape}")
            else:
        ...
        logger.info(f"mean-reduced text_embs shape: {text_embs.shape}")
        if type(self.proj) == ProjectionLayer or type(self.proj) == DoubleMLP:
            text_embs = self.proj.project_clip_txt(text_embs)
            logger.info(f"projected text_embs shape: {text_embs.shape}")
    ...
    def generate_masks(
        ...
        """ 
        logger.info("generate_masks called")
        ...
        logger.info(f"patched image_feat shape: {image_feat.shape}")

        logger.info(f"self.feats keys: {self.feats.keys()}")
        self_attn, self_attn_maps = self.process_self_attention(
            self.feats["self_attn"],
            batch_size,
        ...
        if self.with_bg_clean:
            logger.info("with_bg_clean=True")
        ...
        logger.info(f"interpolated simmap shape: {mask.shape}")
        if apply_pamr:
